...
void squelettisation(int **transformee, t_flag **flags)
{
	...
	for(dist = 1; dist <= max; dist++)
	{
		...
		for(x = 0; x < imgHeight; x++)
		{
			for(y = 0; y < imgWidth; y++)
			{
				if((flags[x][y] | CONTOUR) //Si on est sur un contour
					&& !((flags_N(flags, x, y) ^ flags_S(flags, x ,y)) & (CONTOUR | INTERNE)) //Que nord et sud sont semblables
					&& !((flags_E(flags, x, y) ^ flags_O(flags, x ,y)) & (CONTOUR | INTERNE))) //Que est et ouest sont semblables
					{
						printf("multiple!\n");
					}
			}
